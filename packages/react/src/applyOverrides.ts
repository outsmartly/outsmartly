...
function useForceUpdate() {
  try {
    ...
    // This happens sometimes e.g. when multiple versions of React are included.
    // This feature isn't critical--it's more of a last resort fallback and for
    // use in dev mode later.
    if (!e?.message.startsWith('Invalid hook call.')) {
      console.error(e);
    }
    return () => {};
  }
