...
export default class Deploy extends Command {
  ...
  async bundleAnalysis(tmpDir: string): Promise<Analysis> {
    ...
    try {
      ...
      for (const filePath of fs.readdirSync(analysisDir)) {
      ...
      for (const filename of fs.readdirSync(dependenciesDir)) {
        ...
        const originalRelativePath = content.slice(0, indexOfFirstNewline);
      ...
      const bundle = await rollup({
        input,
        plugins: [rollupVirtual(vfsForRollup), rollupCommonJs()],
      });
      const { output } = await bundle.generate({
        format: 'cjs',
        exports: 'auto',
